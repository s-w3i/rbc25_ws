...
class GoToStartingPointState(State):
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        # publish goal 0.654 -4.329 1.500
        # Position(0.686044, -0.89254, 0), Orientation(0, 0, -0.696329, 0.717723) = Angle: -1.54054
        pub = node.create_publisher(PoseStamped, '/goal_pose', 10)
        goal = PoseStamped()
        goal.header.stamp = node.get_clock().now().to_msg()
        goal.header.frame_id = "map"
        goal.pose.position.x = 0.686044  
        goal.pose.position.y =  -0.90254
...
class StartConversationState(State):
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        time.sleep(5.0)  # Simulate waiting for awake flag
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        while True:
            ...
            if not res or not res.success:
                node.get_logger().error("Segmentation failed. Retrying...")
                time.sleep(1.0)
                continue

            n = len(res.segmented_images)
            if n == 0:
                node.get_logger().error("No humans detected. Retrying...")
                time.sleep(1.0)
                continue

            # ── REPLACED RETAKE WITH NEAREST-SELECTION ──
            if n > 1:
                # pair images with centroids
                paired = list(zip(res.segmented_images, res.centroids))
                # pick the one with minimum distance²
                selected_img, selected_centroid = min(
                    paired,
                    key=lambda ic: ic[1].x**2 + ic[1].y**2 + ic[1].z**2
                )
                node.get_logger().info(
                    f"Multiple humans ({n}), keeping nearest at "
                    f"({selected_centroid.x:.2f}, {selected_centroid.y:.2f}, {selected_centroid.z:.2f})"
                )
            else:
                selected_img = res.segmented_images[0]
                selected_centroid = res.centroids[0]

            # Convert & save
            try:
                cv_img = self.bridge.imgmsg_to_cv2(selected_img, desired_encoding="bgr8")
            except Exception as e:
...
class GoToHostFirstState(State):
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        # publish goal
        # Position(-1.74817, -0.41479, 0), Orientation(0, 0, -0.998353, 0.0573717) = Angle: -3.02679

        pub = node.create_publisher(PoseStamped, '/goal_pose', 10)
        goal = PoseStamped()
        goal.header.stamp = node.get_clock().now().to_msg()
        goal.header.frame_id = "map"
        goal.pose.position.x = -1.74817  
        goal.pose.position.y =  -0.41479
        goal.pose.position.z = 0.0
        yaw = -3.02679 # e.g. 180° facing south in your map
...
class GoToChairDetectionState(State):
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        # Setting goal pose: Frame:map, Position(-1.74817, -0.41479, 0), Orientation(0, 0, -0.998353, 0.0573717) = Angle: -3.02679
        pub = node.create_publisher(PoseStamped, '/goal_pose', 10)
        goal = PoseStamped()
        goal.header.stamp = node.get_clock().now().to_msg()
        goal.header.frame_id = "map"
...
def main():
    ...
    sm.add_state("GO_TO_HOST_FIRST", GoToHostFirstState(), transitions={"nav_done": "INTRODUCE_GUEST_FIRST"})
    ...
    sm.add_state("GO_TO_HOST_SECOND", GoToHostSecondState(), transitions={"nav_done": "INTRODUCE_GUEST_SECOND"})
